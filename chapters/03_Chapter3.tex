...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{images/ch03_fig01_agent_env_loop.png}
\caption{The agent--environment loop: at every timestep the agent selects an action, the environment returns a new state and reward.}
\label{fig:ch03:agentenv}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{images/ch03_fig02_policy_types.png}
\caption{Deterministic policies map each state to a single action; stochastic policies assign a probability distribution over actions.}
\label{fig:ch03:policy}
\end{figure}
...
\texttt{code/notebooks/part1\_foundations/03\_rl\_fundamentals.ipynb}
