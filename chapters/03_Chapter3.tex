...
\begin{examplebox}
\textbf{Prompt:}~\texttt{"Explain machine learning"}

\smallskip
\textbf{Bad strategy} (low reward) -- response:

\textit{``Machine learning is when computers use algorithms to learn from data and make predictions or decisions without being explicitly programmed\ldots''} [continues with jargon]

\smallskip
\textbf{Average reward:} +3 (too technical, not helpful for beginners).
\tcblower
\textbf{Good strategy} (high reward) -- response:

\textit{``Think of machine learning as teaching a child to recognize animals. You show them many pictures and say `that is a dog, that is a cat.' Eventually, they learn the patterns and can identify animals that they have never seen before. Computers do something similar with data!''}

\smallskip
\textbf{Average reward:} +9 (clear, accessible, helpful!).
\end{examplebox}
...
The companion code for this chapter is at \texttt{github.com/arunpshankar/packt-final} -- swap \texttt{github.com} for \texttt{githubtocolab.com} to open it directly in Colab. The diagram below provides a visual summary of the RL concepts covered in this chapter.

\clearpage
\begin{figure}[!h]
\centering
\includegraphics[width=0.88\textwidth]{images/ch03_fig01_rl_overview.png}
\caption{From pre-training to RL fine-tuning -- the bridge from pattern learning to preference learning.}
\label{fig:ch03:rl_overview}
\end{figure}
