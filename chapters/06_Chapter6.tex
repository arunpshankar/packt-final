...
\chapter{RLHF: The Three-Step Dance}
...
\begin{figure}[!h]
\centering
\includegraphics[width=0.88\textwidth]{images/ch06_fig01_rlhf_ppo_intro.png}
\caption{RLHF and PPO at a glance -- collect human preferences, train a reward model, then optimize the policy with PPO.}
\label{fig:ch06:rlhf_ppo_intro}
\end{figure}
...
\begin{tipbox}
\textbf{Watch reward hacking happen in real time.} The companion notebook for this chapter (\texttt{code/notebooks/part2\_core/06\_rlhf\_ppo.ipynb}) is deliberately sized small -- 15 preference pairs, 50 PPO steps -- so the failure mode the previous sections warned about is the \emph{expected} outcome of the demo, not an edge case. After PPO completes, the comparison cell prints the before/after responses with their reward scores side by side. You will typically see all three of these at once:

\begin{itemize}[nosep]
    \item \textbf{Reward improvement of +15 or more} between step 1 and step 50. Looks like a win.
    \item \textbf{Mean KL divergence around 5--10}, far above the \texttt{target\_kl = 0.1} set in the config. The policy has drifted off the leash.
    \item \textbf{Outputs that are noticeably worse}: repetitive fragments like ``Instruction: Instruction'', the model echoing the question back instead of answering it, or factually incorrect responses that nonetheless score \emph{higher} than the clean SFT answer.
\end{itemize}
