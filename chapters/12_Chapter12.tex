...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{images/ch12_fig01_deepseek_phases.png}
\caption{The DeepSeek-R1 five-phase training pipeline alternates between GRPO reinforcement learning and supervised anchoring via rejection sampling.}
\label{fig:ch12:deepseek}
\end{figure}
...
\begin{figure}[htbp]
\centering
\includegraphics[width=0.92\linewidth]{images/ch12_fig02_think_answer_format.png}
\caption{The \texttt{<think>}$\ldots$\texttt{</think>} scratchpad format separates the model's chain-of-thought reasoning from its final answer.}
\label{fig:ch12:format}
\end{figure}
...
\begin{tipbox}
\textbf{Companion notebooks.} Both notebooks are in \texttt{code/notebooks/part3\_advanced/} in the repository at \texttt{github.com/arunpshankar/packt-final}. Replace \texttt{github.com} with \texttt{githubtocolab.com} in the URL to open directly in Colab.
\begin{itemize}
  \item \texttt{12a\_reasoning\_grpo\_concepts.ipynb}
  \item \texttt{12b\_reasoning\_grpo\_deepseek.ipynb}
\end{itemize}
\end{tipbox}
